...
//    }
    ...
    public ResponseEntity<ResponseObject> createPromotion(@RequestBody Promotion promotion) {
        Date date = new Date();
//        if(promotion.getStarttime().before(date) ==true){
//            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
//                    new ResponseObject(HttpStatus.BAD_REQUEST,"Ngày bắt đầu không được nhỏ hơn ngày hiện tại",""));
//        }
//        if(promotion.getEndtime().before(date) ==true){
//            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
//                    new ResponseObject(HttpStatus.BAD_REQUEST,"Ngày kết thúc không được nhỏ hơn ngày hiện tại",""));
//        }
        promotion.setActive(true);
        promotion.setDelete(true);
        Promotion promotionDb =  promotionService.create(promotion);
        return ResponseEntity.status(HttpStatus.OK).body(
                new ResponseObject(HttpStatus.OK,"Tạo thành công",null)
        );
    }

    @PostMapping("/configPromotion")
    public PromotionDetails config(@RequestBody PromotionDetails promotionDetails) {
        return promotionDetaitlsService.save(promotionDetails);
    }

    @GetMapping("/configPromotion/{id}")
    public List<PromotionDetails> getAllconfig(@PathVariable("id") Long id) {
        return promotionDetaitlsService.findByPromotionId(id);
    }
}
